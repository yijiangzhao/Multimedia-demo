...
////////////////////////////////////////////////////////////
//ente
void video_pixmap_widget::drawLines(QImage &dst){
    std::cout<<"draw line"<<std::endl;
    Q_ASSERT(m_netOutputDatum.type == bm::NetOutputDatum::SaftyhatRecogniton);
    if (m_netOutputDatum.safetyhat_objects.size() > 0) {
        QPainter painter(&dst);
        QPen Pen(Qt::blue);
        Pen.setWidth(5);
        painter.setPen(Pen);
        for(int i = 0; i < m_netOutputDatum.safetyhat_objects.size(); ++i) {
            bm::SafetyhatObject pt = m_netOutputDatum.safetyhat_objects[i];
            const QPoint keypoint1{ intRound(pt.x1), intRound(pt.y1) };
            const QPoint keypoint2{ intRound(pt.x2), intRound(pt.y2) };
...
void video_pixmap_widget::drawBox(QImage &dst)
{
    ...
    if (m_netOutputDatum.obj_rects.size() > 0) {
        ...
        for(int i = 0; i < m_netOutputDatum.obj_rects.size(); ++i) {
            ...
            QString text = QString("%1-%2M").arg(pt.track_id).arg(pt.distance, 0, 'g', 3);
            painter1.drawText(pt.x1-1, pt.y1-4, text);
        }

        QPen bluePen(Qt::blue);
        bluePen.setWidth(5);
        painter1.setPen(bluePen);
        if(m_netOutputDatum.obj_rects.size()>0){
            std::vector<cv::Vec4i> ls = m_netOutputDatum.obj_rects[0].lines;
            const QPoint keypoint1{ intRound(ls[0][0]), intRound(ls[0][1]) };
            const QPoint keypoint2{ intRound(ls[0][2]), intRound(ls[0][3]) };
            const QPoint keypoint3{ intRound(ls[1][0]), intRound(ls[1][1]) };
            const QPoint keypoint4{ intRound(ls[1][2]), intRound(ls[1][3]) };
            painter1.drawLine(keypoint1, keypoint2);
            painter1.drawLine(keypoint3, keypoint4);
        }  
    }
