    ...
    void load_config(const char* config_file = "cameras.json") {
        ...
        for(int card_index = 0; card_index < card_num; ++card_index) {
            ...
            for(int i = 0;i < camera_num; ++i) {
                ...
                readHMat(json_url_info["HMat"], card_config.HMat);
                readROI(json_url_info["road_ROI"], card_config.ROI);
            }
...
public:
    ...
    cv::Point * getROI(int index){
        return m_cards[index].ROI;
    }

    cv::Mat getHMat(int index){
        cv::Mat M = m_cards[index].HMat;
        return M;
    }

    //
    void readHMat(Json::Value Array, cv::Mat &HMat){
        cv::Mat tmp(3, 3, CV_64F, cv::Scalar(1));  
        std::cout<< "read configs"<<std::endl;
        std::cout<<"row: "<<Array.size()<<" cols: "<<Array[0].size()<<std::endl;
        ...
        for (int i = 0; i < rows; i++)
        {
            ...
            {
                tmp.at<double>(i, j) = Array[i][j].asDouble();
            }
        }
        HMat = tmp.clone();
    } 
    //
    void readROI(Json::Value Array, cv::Point pt[]){
        pt[0] = cv::Point(Array[0][0].asInt(), Array[0][1].asInt());
        pt[1] = cv::Point(Array[1][0].asInt(), Array[1][1].asInt());
        pt[2] = cv::Point(Array[2][0].asInt(), Array[2][1].asInt());
        pt[3] = cv::Point(Array[3][0].asInt(), Array[3][1].asInt());
    }
