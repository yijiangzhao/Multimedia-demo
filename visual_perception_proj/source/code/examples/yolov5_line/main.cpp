...
const char* APP_ARG_STRING= //"{bmodel | /data/models/yolov5s_4batch_int8.bmodel | input bmodel path}"
                       ...
                       "{max_batch | 1 | Max batch size}"
                        ...
                        "{warningThresh | 10.0 | warning threshold of distance}";
...
int main(int argc, char *argv[])
{
    ...
    for(int card_idx = 0; card_idx < card_num; ++card_idx) {
        ...
        float warning_thresh = parser.get<float>("warningThresh");
        cv::Point *pts = cfg.getROI(card_idx);
        std::cout<<"pts3: "<<pts[3]<<std::endl;
        std::shared_ptr<LaneDetector> laner = std::make_shared<LaneDetector>(pts);
        std::shared_ptr<YoloV5> detector = std::make_shared<YoloV5>(contextPtr, max_batch, warning_thresh);
        detector->set_laneDetector(laner);
        detector->setHMat(cfg.getHMat(card_idx));
