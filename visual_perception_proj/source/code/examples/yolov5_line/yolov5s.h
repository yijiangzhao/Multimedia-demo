...
// #include "line/include/Img.hpp"
// #include "line/include/Edge.h"
// #include "line/include/Hough.h"
// #include "line/include/Kernel.h"
...
public:
    std::shared_ptr<LaneDetector> mlaner;
    cv::Mat HMat;
    cv::Mat HInv;
    YoloV5(bm::BMNNContextPtr bmctx, int max_batch=1, float warning_thresh=10);
    // YoloV5(bm::BMNNContextPtr bmctx, LaneDetector laner, int max_batch=1);
    ~YoloV5();
    ...
    void set_laneDetector(std::shared_ptr<LaneDetector> laner){
        mlaner = laner;
    }
    void calcPosition(bm::NetOutputObject& box);
    void setHMat(cv::Mat H){
        HMat = H.clone();
        cv::invert(HMat, HInv);
    }
private:
